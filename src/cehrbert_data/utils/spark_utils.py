...
def preprocess_domain_table(
    ...
    if domain_table_name == "visit_occurrence":
        ...
        if "discharge_to_concept_id" in domain_table.schema.fieldNames():
            domain_table = domain_table.withColumnRenamed("discharge_to_concept_id",
...
def roll_up_diagnosis(condition_occurrence, concept, concept_relationship):
    ...
    condition_occurrence = (
        condition_occurrence.join(
            condition_icd_hierarchy,
            condition_occurrence["condition_source_concept_id"] == condition_icd_hierarchy[
                "source_concept_id"],
            how="left",
        )
...
def roll_up_procedure(procedure_occurrence, concept, concept_ancestor):
    ...
    procedure_icd = (
        procedure_icd.withColumn("parent_concept_code",
    ...
    procedure_10pcs = (
        procedure_10pcs.withColumn("parent_concept_code",
    ...
    procedure_occurrence = (
        procedure_occurrence.join(
            procedure_hierarchy,
            procedure_occurrence["procedure_source_concept_id"] == procedure_hierarchy[
                "source_concept_id"],
            how="left",
        )
...
def extract_ehr_records(
    ...
    if include_concept_list and patient_ehr_records:
        ...
        qualified_concepts = preprocess_domain_table(spark, input_folder,
                                                     QUALIFIED_CONCEPT_LIST_PATH).select(
            "standard_concept_id"
        )
    ...
    if include_visit_type:
        ...
        patient_ehr_records = patient_ehr_records.join(visit_occurrence,
...
def create_hierarchical_sequence_data(
    ...
    patient_visit_sequence = (
        patent_visit_sequence.withColumn("visit_concept_orders",
                                         F.col("visit_struct_data.visit_concept_order"))
...
def create_visit_person_join(person, visit_occurrence, include_incomplete_visit=True):
    ...
    visit_segment_udf = F.col("visit_rank_order") % F.lit(2) + 1
    visit_windowing = W.partitionBy("person_id").orderBy("visit_start_date", "visit_end_date",
                                                         "visit_occurrence_id")
    ...
    visit_occurrence = visit_occurrence.withColumn("prolonged_stay",
                                                   prolonged_length_stay_logic).select(
        "visit_occurrence_id",
        "visit_concept_id",
        "person_id",
        "prolonged_stay",
...
def get_mlm_skip_domains(spark, input_folder, mlm_skip_table_list):
    ...
    """
    domain_tables = [
        preprocess_domain_table(spark, input_folder, domain_table_name) for domain_table_name in
        mlm_skip_table_list
    ]
