...
from cehrbert_data.const.common import CATEGORICAL_MEASUREMENT, MEASUREMENT
...
class PatientEventBaseDecorator(PatientEventDecorator):
    ...
    def _decorate(self, patient_events: DataFrame):
        ...
        valid_visit_ids = patient_events.select("visit_occurrence_id",
                                                "cohort_member_id").distinct()
        ...
        visits = (
            ...
            .select(
                ...
                F.col("visit_concept_id").cast("int").isin([9201, 262, 8971, 8920]).cast(
                    "int").alias("is_inpatient"),
...
class PatientEventAttDecorator(PatientEventDecorator):
    ...
    def _decorate(self, patient_events: DataFrame):
        ...
        cohort_member_person_pair = patient_events.select("person_id",
                                                          "cohort_member_id").distinct()
        ...
        visit_occurrence = (
            self._visit_occurrence.select(
                ...
                F.coalesce("visit_end_date", "visit_start_date").cast(T.DateType()).alias(
                    "visit_end_date"),
                "visit_concept_id",
                "visit_occurrence_id",
        ...
        inpatient_visits = visit_occurrence.where(
            F.col("visit_concept_id").isin([9201, 262, 8971, 8920])).select(
            "visit_occurrence_id", "visit_end_date", "cohort_member_id"
        )
        inpatient_events = patient_events.join(inpatient_visits,
        ...
        inpatient_events = (
            ...
            .withColumn(
                "date",
                F.when(F.col("date") < F.col("visit_start_date"),
                       F.col("visit_start_date")).otherwise(
                    F.when(F.col("date") > F.col("visit_end_date"),
                           F.col("visit_end_date")).otherwise(F.col("date"))
                ),
            )
                ...
                inpatient_events.withColumn(
                    ...
                    F.row_number().over(
                        W.partitionBy("cohort_member_id", "visit_occurrence_id",
                                      "concept_order").orderBy("priority")
                    ),
                )
...
class DemographicPromptDecorator(PatientEventDecorator):
    ...
    def _decorate(self, patient_events: DataFrame):
        ...
        if self._use_age_group:
            ...
            age_at_first_visit_udf = F.concat(F.lit("age:"),
...
class DeathEventDecorator(PatientEventDecorator):
    ...
    def _decorate(self, patient_events: DataFrame):
        ...
        death_records = patient_events.join(self._death.select("person_id", "death_date"),
                                            "person_id")
        ...
        new_tokens = att_records.unionByName(vs_records).unionByName(death_records).unionByName(
            ve_records)
